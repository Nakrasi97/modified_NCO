...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        if self.is_vrp or self.is_orienteering or self.is_pctsp:
            ...
            if problem.NAME == "motsp":
                step_context_dim = 2 * embedding_dim  # Embedding of first and last node
                node_dim = 2  # x1, y1, x2, y2
            else:
    ...
    def _precompute(self, mixed_embeddings, num_steps=1):
        ...
        # embeddings = torch.cat([obj1_embeddings, obj2_embeddings], dim=-1)
        ...
        if self.is_vrp:
            ...
            if num_steps == 1:  # We need to special case if we have only 1 step, may be the first or not
                if state.i.item() == 0:
                    ...
                    visited = torch.stack(state.visited_nodes, dim=1)
                    visited_rep = visited[:, :, None].expand(batch_size, visited.size(-1), embeddings.size(-1))
                    visited_embedding = embeddings.gather(1, visited_rep).mean(1).unsqueeze(1)

                    current_embedding = embeddings.gather(1, current_node[:, :, None]
                                                          .expand(batch_size, 1, embeddings.size(-1)))
                    return torch.cat([visited_embedding, current_embedding], dim=-1)
